...
def draw_user_b30(obj_in: UserBest30) -> Image.Image:
    ...
    for i in range(min(b30_len, 15)):
        ...
        ylen = 60 * i
        draw.text(
            (150, ylen + inner_gap[0]),
            f'{b30_songids[i]} [{["PST", "PRS", "FTR", "BYD"][b30_difficulties[i]]}]',
            title_color,
            font=ft_title,
        )
        draw.text(
            (150, ylen + inner_gap[1]),
            f"Score: {b30_scores[i]}",
            (255, 255, 255),
            font=ft_normal,
        )
        draw.text(
            (400, ylen + inner_gap[2]),
            f"PTT: {b30_ratings[i]:.2f}",
            (255, 255, 255),
            font=ft_normal,
        )
    if b30_len > 15:
        ...
        for i in range(b30_len - 15):
            ...
            draw.text(
                ...
                f'{b30_songids[i + 15]} [{["PST", "PRS", "FTR", "BYD"][b30_difficulties[i]]}]',
                title_color,
                font=ft_title,
            )
            draw.text(
                (600, ylen + inner_gap[1]),
                f"Score: {b30_scores[i + 15]}",
                (255, 255, 255),
                font=ft_normal,
            )
            draw.text(
                (850, ylen + inner_gap[2]),
                f"PTT: {b30_ratings[i + 15]:.2f}",
                (255, 255, 255),
                font=ft_normal,
            )
