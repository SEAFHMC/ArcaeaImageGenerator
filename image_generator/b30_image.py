...
def draw_score_detail(
    ...
    write_song_name = text_image(
        song_name, StaticPath.kazesawa_regular, 40, average_color, contrast_degree
    )
    image.alpha_composite(write_song_name, (45, 32))
    write_score = text_image(
        f'{data["score"]:,}', StaticPath.exo_medium, 40, average_color, contrast_degree
    )
    image.alpha_composite(write_score, (45, 80))
    write_ranking = text_image(
        f"#{rank + 1}",
    ...
    write_PTT = text_image(
        "PTT", StaticPath.exo_medium, 25, average_color, contrast_degree
    )
    image.alpha_composite(write_PTT, (250, 130))
    write_DATE = text_image(
        "DATE", StaticPath.exo_medium, 25, average_color, contrast_degree
    )
    ...
    # Count
    write_p_count = text_image(
        str(data["perfect_count"]),
    ...
    write_sp_count = text_image(
        f'+{data["shiny_perfect_count"]}',
        StaticPath.kazesawa_regular,
        20,
        average_color,
        contrast_degree,
    ...
    if True:
        for num, data in enumerate(score_info_list):
            ...
            B30_bg.alpha_composite(
                draw_score_detail(data, rank=num, song_id=data["song_id"], mask=mask),
